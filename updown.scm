...
(receive (options operands)
  ...
  (for-each (lambda (l)
            ...
            options) 
  (define reset-n (string->number (or (alist-ref 'reset-length options) "25")))
  (define input-rgx (irregex (or (alist-ref 'input options) "^(?:\x1b\\[[0-9;]+m)?\\[(.+)\\](?:\x1b\\[[0-9;]+m)?(.+)$")))
  (define output-fmt (alist-ref 'format options))
  (define default-prefix (alist-ref 'default-prefix options))
